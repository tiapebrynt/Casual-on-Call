...
{
    /** Releases a confirmed payment to the worker exactly once and notifies both parties. */
    public function markPaid(Payment $payment, string $method, string $reference): Payment
    {
        return DB::transaction(function () use ($payment, $method, $reference): Payment {
            $payment = Payment::with(['application.job.company.user', 'application.worker.user'])->lockForUpdate()->findOrFail($payment->id);
            ...
            $worker->notify(new WorkflowNotification(
            ...
            $companyUser = $payment->application->job->company->user ?? null;
            if ($companyUser) {
                $companyUser->notify(new WorkflowNotification(
                    'Pembayaran Invoice Berhasil',
                    'Pelunasan gaji Invoice #' . $payment->invoice_number . ' untuk ' . $payment->application->worker->user->name . ' berhasil diproses.',
                    route('payments.show', $payment)
                ));
            }
