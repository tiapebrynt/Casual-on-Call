...
class JobController extends Controller
{
    ...
    {
        $filters = $request->only(['search', 'category', 'location', 'min_rate', 'max_rate', 'rate_range']);
        return view('jobs.index', [
            'jobs' => $this->jobs->paginatePublished($filters),
    ...
    {
        $job->load(['company.user', 'category', 'requirements'])->loadCount('applications');
        return view('jobs.show', compact('job'));
    }

    public function manage(Request $request): View
    {
        $query = Job::with(['category', 'requirements'])->withCount('applications');
    ...
    {
        abort_unless($request->user()->hasRole('admin') || $job->company_id === $request->user()->company?->id, 403);
